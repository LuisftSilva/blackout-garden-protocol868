(() => {
  ...
  const DEFAULT = {
    version: 4, day: 1, completed: 0, highScore: 0,
  ...
  const game = {
    t:0, cam:{x:0,y:0,tx:0,ty:0}, mission:null,
  ...
  function showMenu(){
    ...
    ui.innerHTML=`<div class="screen"><div class="menu-card">
      ...
      <p class="subtitle">Portugal ficou às escuras. Tu controlas uma base técnica escondida na serra, reativas relés 868 MHz, recolhes recursos e manténs a rede Mesh viva. Agora em modo vertical para telemóvel.</p>
      ...
      <div class="grid3"><div class="panel"><h3>Explorar</h3><p>Missões verticais, nevoeiro, chuva, drones e extração.</p></div><div class="panel"><h3>Disparar melhor</h3><p>Botão de tiro com auto-mira ao inimigo mais próximo. Toca no mapa para definir alvo manual.</p></div><div class="panel"><h3>Melhorar</h3><p>Investe em reator, filtragem, estufa, rede, drone e oficina.</p></div></div>
    </div></div>`;
  ...
  function showBase(){
    ...
    ui.innerHTML=`<div class="screen"><div class="base-card">
      ...
      <div class="grid2"><div class="panel"><h3>Controlos</h3><p>Joystick para mover. Mantém <b>TIRO</b> pressionado para disparar com auto-mira. Toca num inimigo/zona para mirar manualmente. <b>SCAN</b> revela alvos e <b>USAR</b> ativa terminais.</p></div><div class="panel"><h3>Objetivo</h3><p>Ativa todos os terminais, levanta a antena 868 MHz e volta ao ponto de extração.</p></div></div>
  ...
  function buildMission(m){
    ...
    game.drone={x:game.player.x-35,y:game.player.y+20,cd:0,t:0}; game.cam.x=game.player.x-W/2; game.cam.y=game.player.y-H*.72;
  ...
  function worldToScreen(x,y){ return {x:x-game.cam.x,y:y-game.cam.y}; }
  function screenToWorld(x,y){ return {x:x+game.cam.x,y:y+game.cam.y}; }
  ...
  function update(dt){
    ...
    game.cam.tx=clamp(p.x-W/2,0,Math.max(0,game.mission.w-W)); game.cam.ty=clamp(p.y-H*.68,0,Math.max(0,game.mission.h-H)); game.cam.x=lerp(game.cam.x,game.cam.tx,.10); game.cam.y=lerp(game.cam.y,game.cam.ty,.10); shake=Math.max(0,shake-dt*16);
    updateHUD();
  }
  ...
  function drawMission(){
    ...
    const bg=ctx.createLinearGradient(0,0,0,H); bg.addColorStop(0,m.clima==='neve'?'#071217':m.clima==='cinza'?'#11100b':'#07120d'); bg.addColorStop(1,'#050906'); ctx.fillStyle=bg; ctx.fillRect(0,0,W,H);
    ...
    ctx.restore(); drawWeather(); drawMinimap(); }
